...
app.get("/lastuser", async (req, res)=> {
  const query = "SELECT idUser,nom, telephone,date_inscription FROM `utilisateurs` ORDER BY idUser DESC LIMIT 10";
  pool.query(query, (error, results) => {
    if(!results[0]){
      res.json({status: "Not Found!"});
    }else{
...
app.get("/scan", async (req, res)=> {
  const query = "SELECT U.idUser, nom, telephone, COUNT(S.idScan) AS scan, (COUNT(S.idScan)*400) AS merite FROM utilisateurs U, scan S, qr_codes Q WHERE S.idCode = Q.idCode AND S.idUser = U.idUser AND S.valider = 0 GROUP BY S.idUser ORDER BY COUNT(S.idScan) DESC";
  pool.query(query, (error, results) => {
    if(!results[0]){
      res.json({status: "Not Found!"});
    }else{
